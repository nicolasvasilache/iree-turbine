...
def build_block_constraints(*args, **kwargs) -> Sequence[tkw.Constraint]:
    ...
    constraints += [tkw.WorkgroupConstraint(D2, BLOCK_D2, 1)]
    constraints += [tkw.WaveConstraint(D2, BLOCK_D2)]
...
def torch_naive_moe_step_1(a, w1, w2, score, topk):
    ...
    for i in range(w1.shape[0]):
        mask = topk_ids == i
        if mask.sum():
            out[mask] = SiluAndMul()(a[mask] @ w1[i].transpose(0, 1))
    return topk_weight, topk_ids, tmp_2
...
if __name__ == "__main__":
    ...
    def static_correct_1():
        ...
        topk_weight, topk_ids, tmp_2 = torch_naive_moe_step_1(a, w1, w2, score, topk)

        cfg = config()
        executable_kernel = get_fused_moe_kernel(build_block_constraints(cfg))
        run_cfg = get_default_run_config()
        with tk.gen.TestLaunchContext(
            cfg["static_symbols"], canonicalize=True, run=True, run_config=run_cfg
        ):
